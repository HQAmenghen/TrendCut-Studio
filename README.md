...
<p align="center">
  ...
  <img alt="License" src="https://img.shields.io/badge/license-unpublished-lightgrey">
</p>
...
```powershell
git clone https://gitee.com/HQAmenghen/TrendCut-Studio.git
cd TrendCut-Studio
```
...
3. 修改 Python 脚本时，保留可被 Node 解析的结构化输出和明确退出码。
...
该接口会检查关键环境变量、目录、Python、FFmpeg 和 Playwright Python 环境。
...
当前仓库尚未包含正式开源许可证文件。使用、分发或二次开发前，请先与仓库维护者确认授权范围。若计划作为正式开源项目发布，建议补充 `LICENSE`、`CONTRIBUTING.md` 和安全披露说明。
