    ...
    try:
        ...
        for account in accounts:
            ...
            if not normalized or normalized in seen:
                continue
            cleaned_accounts.append(normalized)
            seen.add(normalized)
        return cleaned_accounts or DEFAULT_ACCOUNTS
...
5. Return at most {MAX_CANDIDATES_PER_ACCOUNT} candidate posts.
6. If nothing matches, return an empty items array.
...
def main() -> int:
    ...
    ranked = sorted(enriched, key=final_sort_key, reverse=True)[:10]
