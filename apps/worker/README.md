...
- `script_worker` runs the legacy `ScriptRewriterSkill` in-process and records the structured skill output.
- ASR, material scoring, clip plan, render, review, publish, and RPA job types still use adapter manifests until their concrete executors are migrated.
