...
class QwenFiletransAsrTest(unittest.TestCase):
    ...
    def test_reference_text_authority_accepts_direct_llm_subtitle_json(self):
        subtitles = [
            {"time": [0.0, 1.2], "zh": "support holders", "text": "support holders"},
            {"time": [1.2, 2.8], "zh": "protect Bitcoin", "text": "protect Bitcoin"},
        ]
        reference = [
            {"time": [0.0, 2.8], "zh": "支持加密持有者自我托管权，保护比特币未来"}
        ]
        llm_items = [
            {"time": [0.0, 1.2], "zh": "支持加密持有者自我托管权，"},
            {"time": [1.2, 2.8], "zh": "保护比特币未来"},
        ]

        class FakeResponse:
            text = json.dumps(llm_items, ensure_ascii=False)
        ...
        self.assertEqual(authoritative, run_asr.normalize_final_subtitles(llm_items))
    ...
    def test_reference_text_authority_retries_invalid_llm_json_in_strict_mode(self):
        subtitles = [
            {"time": [0.0, 1.0], "zh": "support holders", "text": "support holders"},
            {"time": [1.0, 2.0], "zh": "protect Bitcoin", "text": "protect Bitcoin"},
        ]
        reference = [
            {"time": [0.0, 2.0], "zh": "支持持有者，保护比特币"}
        ]
        responses = [
            json.dumps([{"time": [0.0, 2.0], "zh": "support holders"}], ensure_ascii=False),
            json.dumps([
                {"time": [0.0, 1.0], "zh": "支持持有者，"},
                {"time": [1.0, 2.0], "zh": "保护比特币"},
            ], ensure_ascii=False),
        ]
    ...
    def test_reference_text_authority_strict_mode_raises_after_invalid_llm_retries(self):
        subtitles = [
            {"time": [0.0, 1.0], "zh": "support holders", "text": "support holders"},
            {"time": [1.0, 2.0], "zh": "protect Bitcoin", "text": "protect Bitcoin"},
        ]
        ...
        class FakeResponse:
            text = json.dumps([{"time": [0.0, 2.0], "zh": "support holders"}], ensure_ascii=False)
    ...
    def test_reference_text_authority_non_strict_invalid_llm_returns_empty(self):
        subtitles = [
            {"time": [0.0, 1.0], "zh": "support holders", "text": "support holders"},
            {"time": [1.0, 2.0], "zh": "protect Bitcoin", "text": "protect Bitcoin"},
        ]
    ...
    def test_reference_text_authority_without_llm_returns_normalized_reference_subtitles(self):
        subtitles = [
            {"time": [0.0, 1.0], "zh": "raw asr", "text": "raw asr"},
            {"time": [1.0, 2.0], "zh": "more raw asr", "text": "more raw asr"},
        ]
        reference = [
            {"time": [1.0, 2.0], "zh": "第二段参考"},
            {"time": [0.0, 1.0], "zh": "第一段参考"},
        ]

        with patch("pipeline.run_asr.create_llm_client", side_effect=AssertionError("LLM should not run")):
            authoritative = run_asr.build_reference_authority_subtitles(
                subtitles,
                reference,
                {"max_visible_chars": 26},
        ...
        self.assertEqual([item["zh"] for item in authoritative], ["第一段参考", "第二段参考"])
        self.assertEqual([item["time"] for item in authoritative], [[0.0, 1.0], [1.0, 2.0]])
