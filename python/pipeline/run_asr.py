...
from pipeline import asr_filetrans
from pipeline import asr_filetrans_parse
from pipeline.asr_filetrans import (
    DEFAULT_QWEN_ASR_MODEL,
    LEGACY_QWEN_ASR_MODEL,
)
try:
    from pipeline import reference_authority as reference_authority_module
    from pipeline.reference_authority import ReferenceAuthorityAlignmentError
except ImportError:
    import reference_authority as reference_authority_module
    from reference_authority import ReferenceAuthorityAlignmentError
try:
...
def get_qwen_asr_model():
    return asr_filetrans.get_qwen_asr_model()


def get_qwen_asr_api_base_url():
    return asr_filetrans.get_qwen_asr_api_base_url()


def is_public_http_url(value):
    return asr_filetrans.is_public_http_url(value)


def is_truthy_env(name, default=False):
    return asr_filetrans.is_truthy_env(name, default=default)


def normalize_oss_prefix(prefix):
    return asr_filetrans.normalize_oss_prefix(prefix)


def get_oss_signed_url_expires_seconds():
    return asr_filetrans.get_oss_signed_url_expires_seconds()


def get_oss_filetrans_config():
    return asr_filetrans.get_oss_filetrans_config()


def make_oss_object_key(local_file, prefix=""):
    return asr_filetrans.make_oss_object_key(local_file, prefix=prefix)


def create_oss_bucket(config):
    return asr_filetrans.create_oss_bucket(config)
...
def collect_transcripts(payload):
    return asr_filetrans_parse.collect_transcripts(payload)


def sentence_time_range(sentence):
    return asr_filetrans_parse.sentence_time_range(sentence, parse_seconds)


def should_insert_filetrans_space(current_text, token):
    return asr_filetrans_parse.should_insert_filetrans_space(current_text, token)


def join_filetrans_tokens(tokens):
    return asr_filetrans_parse.join_filetrans_tokens(tokens)


def reconstruct_filetrans_words(words):
    return asr_filetrans_parse.reconstruct_filetrans_words(words)


def sentence_text(sentence):
    return asr_filetrans_parse.sentence_text(sentence, apply_domain_corrections)


def parse_word_seconds(word, start_keys, *, default=None):
    return asr_filetrans_parse.parse_word_seconds(word, start_keys, parse_seconds, default=default)


def sentence_words(sentence):
    return asr_filetrans_parse.sentence_words(sentence, parse_seconds, MockWord)


def parse_filetrans_result_segments(payload, include_words=False):
    return asr_filetrans_parse.parse_filetrans_result_segments(
        payload,
        parse_seconds=parse_seconds,
        apply_domain_corrections=apply_domain_corrections,
        infer_language_from_text=infer_language_from_text,
        word_factory=MockWord,
        include_words=include_words,
    )


def fetch_json(url, headers=None):
    return asr_filetrans.fetch_json(url, headers=headers)
...
def submit_qwen_filetrans_task(file_url, model):
    return asr_filetrans.submit_qwen_filetrans_task(file_url, model)


def wait_qwen_filetrans_task(task_id, headers):
    return asr_filetrans.wait_qwen_filetrans_task(task_id, headers)
...
def create_reference_authority_deps():
    return reference_authority_module.ReferenceAuthorityDeps(
        resolve_split_config=resolve_split_config,
        reference_authority_display_limit=reference_authority_display_limit,
        merge_reference_authority_micro_asr_fragments=merge_reference_authority_micro_asr_fragments,
        ...
        get_text_model_for_provider=get_text_model_for_provider,
        create_llm_client=create_llm_client,
        generate_content=generate_content,
        get_text_llm_provider=get_text_llm_provider,
        emit_stage=emit_stage,
        append_debug_event=append_reference_authority_debug_event,
    )


def append_reference_authority_debug_event(event):
    return reference_authority_module.append_reference_authority_debug_event(event)


def reference_authority_asr_debug_segments(asr_entries):
    return reference_authority_module.reference_authority_asr_debug_segments(
        asr_entries,
        create_reference_authority_deps(),
    )


def reference_authority_retry_count():
    return reference_authority_module.reference_authority_retry_count()
...
def build_reference_authority_prompt(asr_entries, reference_text, source_language="", split_config=None, retry_index=0, failure_reason=""):
    return reference_authority_module.build_reference_authority_prompt(
        asr_entries,
        reference_text,
        create_reference_authority_deps(),
        source_language=source_language,
...
def parse_direct_reference_authority_subtitles(results, reference_text):
    return reference_authority_module.parse_direct_reference_authority_subtitles(
        results,
        reference_text,
        create_reference_authority_deps(),
    )
...
def validate_direct_reference_authority_subtitles(results, reference_text):
    return reference_authority_module.validate_direct_reference_authority_subtitles(
        results,
        reference_text,
        create_reference_authority_deps(),
    )
...
def align_reference_authority_with_llm(asr_entries, reference_text, source_language="", split_config=None, strict=False):
    return reference_authority_module.align_reference_authority_with_llm(
        asr_entries,
        reference_text,
        create_reference_authority_deps(),
        source_language=source_language,
...
def build_reference_authority_subtitles(
    ...
    return reference_authority_module.build_reference_authority_subtitles(
        asr_subtitles,
        reference_subtitles,
        create_reference_authority_deps(),
        split_config=split_config,
