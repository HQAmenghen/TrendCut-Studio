...
def parse_direct_reference_authority_subtitles(results, reference_text):
    ...
    for item in results:
        ...
        if not time_range:
            return []
        text = apply_domain_corrections(str(item.get("zh") or item.get("text") or "").strip())
        if not text:
            return []
        entry = {
            "time": [time_range[0], time_range[1]],
...
def validate_direct_reference_authority_subtitles(results, reference_text):
    ...
    subtitles = parse_direct_reference_authority_subtitles(results, reference)
    if not subtitles:
        return []
...
def align_reference_authority_with_llm(asr_entries, reference_text, source_language="", split_config=None, strict=False):
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            results = parse_json_array_from_text(getattr(response, "text", response))
            validated = validate_direct_reference_authority_subtitles(results, reference)
            if not validated:
