...
TrendCut Studio 提供一套本地自动化接口，供 MCP 客户端或其他受控自动化程序调用。该能力的目标不是开放一个裸 API，而是把“找热点、写口播、匹配素材、混剪、审核、发布”整理成可被 AI coding 工具稳定调用的工作流。它由三层组成：

1. Agent API：位于 `/api/agent/v1/*` 的 Token 鉴权 HTTP 接口。
2. MCP bridge：位于 `mcp-server/`，把 Agent API 封装为 MCP tools。
3. Skill guide：描述自然语言意图如何选择工具、如何衔接步骤，以及哪些动作需要人工确认。

开发工作区中可能存在 `.agents/skills/video-assistant-agent/`。它是本地 Skill 使用说明，不是应用运行所需代码。公开项目不发布个人助手配置目录，但会保留 MCP/Agent/Skill 的设计说明，说明这套工作流如何被 AI coding 工具调用。
...
| Local skill guide | `.agents/skills/video-assistant-agent/SKILL.md` | 本地 Skill 使用说明，描述自然语言意图应选择哪个 MCP tool，以及工具之间如何衔接。 |
...
| 口播驱动生产 | `generate_video_from_post`, `generate_video_from_rank`, `generate_narration_from_post`, `generate_narration_from_rank`, `get_job_status`, `get_workflow_next_actions`, `summarize_job_status`, `preview_generated_video` |
| 口播出镜与动作断点 | `get_narration_draft`, `revise_narration_draft`, `update_avatar_render_config`, `generate_avatar_video`, `generate_avatar_video_with_runninghub`, `get_avatar_status`, `preview_avatar_video` |
...
`.agents/skills/video-assistant-agent/` 目录的作用是给本地 MCP 客户端提供工具选择说明。它描述“用户说什么话时，应调用哪类 MCP tool”，不承载业务逻辑。换句话说，MCP 提供可执行工具面，Skill 把这些工具组织成更接近自然语言工作流的使用方式，例如先取榜单、生成口播稿、确认脚本、触发素材匹配混剪、再进入审核和发布草稿。
...
| 不需要口播出镜，直接竖屏 | `create_direct_vertical_video` 或 `create_no_avatar_vertical_video` |
| 查看口播出镜生成进度 | `get_avatar_status` |
...
| 口播与出镜素材 | `GET /api/agent/v1/jobs/:jobId/narration`, `POST /api/agent/v1/jobs/:jobId/narration/revise`, `POST /api/agent/v1/jobs/:jobId/avatar/config`, `POST /api/agent/v1/jobs/:jobId/avatar/generate`, `GET /api/agent/v1/jobs/:jobId/avatar`, `GET /api/agent/v1/jobs/:jobId/avatar/preview` |
