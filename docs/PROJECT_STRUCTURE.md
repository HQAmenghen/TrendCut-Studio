...
- `src/composables/`
  - 业务状态、恢复逻辑和 API 调用封装。
- `src/composables/publishCenter/`
  - 发布中心领域规则，包括平台账号、平台卡片、自动托管计划和展示摘要。
...
- `services/`
  - 业务服务层。
- `services/system/scheduler.js`
  - 后台调度组合根。
- `services/system/scheduler*.js`
  - AutoPilot、定时发布、清理和登录检测等具体后台调度模块。
- `core/`
...
`aiman.mp4` 是内部口播出镜视频文件名，属于运行协议，不代表产品名称。
